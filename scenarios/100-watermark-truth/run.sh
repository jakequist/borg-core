...
# and the recomputed value's `landed at` must name a layer **on the fork**, which is false the moment
...
check_watermark() {
    ...
    # `landed at` — the layer on *this* branch whose membership carried the event here.
    # `authored at` would name the layer the event was first written on, which for an inherited
    # value is a layer on main, and would make this check pass for the wrong reason.
    replayed_at="$(field "$replayed" "landed at")"

    # **The check that the check is real.** A fork inherits its parent's derived layers, so a value
    # that was never recomputed here comes back landed at one of *main's* layers. Only a layer
    # belonging to the fork proves the producer ran again.
    if ! borg layer list --branch "$branch" | grep -q "^$replayed_at[[:space:]]"; then
        fail "$cell was inherited from main, not recomputed: the replay reports it landed at
